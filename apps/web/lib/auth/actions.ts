...
import { isAdminPortalPath } from "@/lib/auth/admin-paths";
...
import { NEXT_INTL_LOCALE_COOKIE_NAME } from "@/lib/i18n/locale-constants";
...
function postAuthPath(next: string, roles: string[]): string {
  ...
  if (pathOnly === "/" || pathOnly.startsWith("/login") || pathOnly.startsWith("/register")) {
    return roles.includes("admin") ? "/dashboard" : "/my";
  }
...
export async function loginPasswordAction(
  ...
  revalidatePath(`/${locale}${destPath}`, "layout");
